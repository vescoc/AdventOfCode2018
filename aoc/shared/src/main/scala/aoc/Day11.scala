...
object Day11 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def powerGridXY(x: Int, y: Int): Int = {
      val X = x + 1
      val Y = y + 1
    ...
    val sumUpLeftSolvers = {
      val sumUpLeft = Array.ofDim[Int](GridWidth, GridHeight)
      ...
      for {
        ...
        sumUpLeft(x)(y) = powerGridXY(x, y) - value(x - 1)(y - 1) + value(x - 1)(y) + value(x)(y - 1)
      }

      def solve(range: Range): Info = {
        @tailrec
    ...
    val powerGridSolvers = {
      ...
      List(solveOpt2 _, solveOpt _, solveBrutal _)
    }
